...
def typed_chars(el, txt, cps, pauses):
    # pauses: [{"at":index,"dur":sec}] pause after 'at' chars typed
    k=0; tleft=el
    ps=sorted(pauses,key=lambda x:x['at'])
    pos=0
    for pz in ps:
        seg=pz['at']-pos
        need=seg/cps
        if tleft<need: return min(len(txt), pos+int(tleft*cps))
        tleft-=need; pos=pz['at']
        if tleft<pz['dur']: return pos
        tleft-=pz['dur']
    return min(len(txt), pos+int(tleft*cps))
_fc={}
def fit_font(txt):
    if txt in _fc: return _fc[txt]
    size=30
    while size>18:
        f=ImageFont.truetype(FONT,size)
        if f.getlength(txt)+PAD+18+PAD+BTN+PAD <= W-80: break
        size-=1
    _fc[txt]=ImageFont.truetype(FONT,size); return _fc[txt]
font=None
def draw_box(d, txt, cursor_on, blank=False):
    ...
    if cursor_on:
        cx=x0+PAD+tw+4
        d.rectangle([cx,ty+4,cx+2,ty+fh+4],fill=(245,245,250,255))
    # play button
...
for i in range(n):
    ...
    for w in wins:
        if w['start']<=t<w['end']:
            ...
            k=typed_chars(t-w['start'],txt,cps,w.get('pauses',[])) if not w.get('empty') else 0
            shown=txt[:k]
            font=fit_font(txt)
            # fade in/out 0.2s
